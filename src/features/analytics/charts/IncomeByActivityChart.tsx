...
import type { ActivityTotal } from "@/lib/analytics-types";

interface IncomeByActivityChartProps {
  data: ActivityTotal[];
}

const PIE_MAX_SLICES = 8;

export function IncomeByActivityChart({ data }: IncomeByActivityChartProps) {
  const hasData = data.length > 0;
  const usePie = data.length > 0 && data.length <= PIE_MAX_SLICES;

  return (
    <ChartCard
      title="Income by activity type"
      description="Income transactions and positive farming session profit"
    >
      {!hasData ? (
        <AnalyticsEmptyState message="No income activity in this time range." />
      ) : usePie ? (
